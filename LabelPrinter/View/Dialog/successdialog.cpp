#include "successdialog.h"
#include "stylesheet.h"
#include "global.h"

#include <QPushButton>
#include <QLabel>
#include <QKeyEvent>
#include <QBoxLayout>

SuccessDialog::SuccessDialog(QWidget *parent) : QDialog(parent)
{
    this->setWindowFlags(Qt::Dialog | Qt::FramelessWindowHint);
    this->setModal(true);
    this->setFixedSize(SUCCESS_DIALOG_SIZE);
    this->setStyleSheet(DIALOG_QSS);
    setSystemMenu();
    setWorkSheet();
}

void SuccessDialog::setSystemMenu(void)
{
    /* 상단 시스템 메뉴 바 위젯 */
    QWidget *pSysTemWidget = new QWidget(this);
    pSysTemWidget->resize(QSize(this->width(), SYSTEM_BAR_HEIGHT));
    pSysTemWidget->setStyleSheet(SYSTEM_MENU_QSS);

    /* 왼쪽 상단 회사로고 아이콘 */
    QLabel *pLogoIcon = new QLabel(pSysTemWidget);
    pLogoIcon->setFixedWidth(LOGO_ICON_WIDTH);
    pLogoIcon->setStyleSheet(LABEL_LOGO_ICON_QSS);

    QLabel *pTitleText = new QLabel(SUCCESS_DIALOG_TITLE_TEXT, pSysTemWidget);
    pTitleText->setStyleSheet(LABEL_TITLE_TEXT_QSS);

    /* 프로그램 종료 버튼 */
    QPushButton *pQuitBtn = new QPushButton(pSysTemWidget);
    pQuitBtn->setFocusPolicy(Qt::NoFocus);
    pQuitBtn->setFixedSize(SYSTEM_BAR_BTN_SIZE);
    pQuitBtn->setIcon(X_MARK_ICON_PATH);
    pQuitBtn->setIconSize(SYSTEM_BAR_BTN_ICON_SIZE);
    pQuitBtn->setStyleSheet(SYSTEM_MENU_QUIT_BTN_QSS);

    /* 수평 레이아웃 설정 */
    QHBoxLayout * pSysTemLayout = new QHBoxLayout;
    pSysTemLayout->setAlignment(Qt::AlignRight);
    pSysTemLayout->setContentsMargins(SYSTEN_BAR_LAYOUT_MARGIN);
    pSysTemLayout->setSpacing(0);
    pSysTemLayout->addWidget(pLogoIcon, 0, Qt::AlignLeft);
    pSysTemLayout->addSpacing(7);
    pSysTemLayout->addWidget(pTitleText, 1, Qt::AlignLeft);
    pSysTemLayout->addWidget(pQuitBtn);
    pSysTemWidget->setLayout(pSysTemLayout);

    QObject::connect(pQuitBtn, SIGNAL(clicked()), this, SLOT(close()));
}

void SuccessDialog::setWorkSheet(void)
{
    QWidget *worksheet = new QWidget(this);
    worksheet->setGeometry(this->x(), SYSTEM_BAR_HEIGHT, this->width(), this->height()-SYSTEM_BAR_HEIGHT);
    worksheet->setStyleSheet(DIALOG_WIDGET_QSS);

    m_pIconLable =  new QLabel(this);
    m_pIconLable->setFixedWidth(SUCCESS_DIALOG_ICON_WIDTH);

    m_pMsg = new QLabel(worksheet);
    m_pMsg->setAlignment(Qt::AlignCenter);
    m_pMsg->setStyleSheet(MSG_LABEL_QSS);

    QHBoxLayout *pMessageHLayout = new QHBoxLayout;
    pMessageHLayout->setContentsMargins(SUCCESS_DIALOG_HLAYOUT_MARGINS);
    pMessageHLayout->addWidget(m_pIconLable);
    pMessageHLayout->addWidget(m_pMsg, 0, Qt::AlignLeft);

    m_pOkBtn = new QPushButton(worksheet);
    m_pOkBtn->setFocusPolicy(Qt::NoFocus);
    m_pOkBtn->setCursor(Qt::PointingHandCursor);
    m_pOkBtn->setFixedSize(DIALOG_BTN_SIZE);
    m_pOkBtn->setText(OK_TEXT);
    m_pOkBtn->setStyleSheet(DIALOG_BTN_QSS);

    QVBoxLayout *pVLayout = new QVBoxLayout;
    pVLayout->addLayout(pMessageHLayout);
    pVLayout->addWidget(m_pOkBtn, 0, Qt::AlignRight);
    worksheet->setLayout(pVLayout);

    QObject::connect(m_pOkBtn,  SIGNAL(clicked()), this, SLOT(close()));
}

void SuccessDialog::setMsg(const QString msg)
{
    m_pMsg->setText(msg);
}

void SuccessDialog::setIcon(bool state)
{
    if(state == true) {
        m_pIconLable->setStyleSheet(OK_ICON_LABEL);
    }
    else {
        m_pIconLable->setStyleSheet(WARNING_ICON_LABEL);
    }
}

void SuccessDialog::closeEvent (QCloseEvent *)
{
    m_pOkBtn->click();
}

void SuccessDialog::keyPressEvent(QKeyEvent *event)
{
    switch(event->key())
    {
        case Qt::Key_Return:
        case Qt::Key_Enter:
        case Qt::Key_Space:
        case Qt::Key_Escape:
            this->close();
            break;
    }
}

void SuccessDialog::mousePressEvent(QMouseEvent *event)
{
    if(event->button() == Qt::LeftButton)
    {
        m_bMoving = true;
        m_pointLastMousePos = event->pos();
    }
}

void SuccessDialog::mouseMoveEvent(QMouseEvent *event)
{
    if(event->buttons().testFlag(Qt::LeftButton) && m_bMoving)
    {
        if (m_pointLastMousePos.y() < MOUSE_MOVE_AREA)
        {
            this->move(this->pos() + (event->pos() - m_pointLastMousePos));
        }
    }
}

void SuccessDialog::mouseReleaseEvent(QMouseEvent *event)
{
    if(event->button() == Qt::LeftButton)
    {
        m_bMoving = false;
    }
}
